from sqlalchemy import Column, String, Integer, DECIMAL, TIMESTAMP, func
...
class Application(Base):
    ...
    pan_number = Column(
        String(10),
        nullable=False,
        comment="Applicant's PAN number"
    )

    applicant_name = Column(
        String(255),
        nullable=True,
        comment="Applicant's full name"
    )
    ...
    loan_amount_inr = Column(
        DECIMAL(12, 2),
        nullable=False,
        comment="Requested loan amount in INR"
    )
    ...
    cibil_score = Column(
        Integer,
        nullable=True,
        comment="Simulated CIBIL score (300-900)"
    )
